...
export const TodoList = ({
  ...
  const getTabInfo = () => {
    switch (activeTab) {
      case 'pending':
        return { label: t('pending'), count: pendingTodos.length, icon: Circle, color: isDarkMode ? 'text-orange-400' : 'text-orange-500', bgColor: isDarkMode ? 'bg-orange-900' : 'bg-orange-50', borderColor: 'border-orange-500' };
      case 'completed':
        return { label: t('completed'), count: completedTodos.length, icon: CheckCircle, color: isDarkMode ? 'text-green-400' : 'text-green-500', bgColor: isDarkMode ? 'bg-green-900' : 'bg-green-50', borderColor: 'border-green-500' };
      case 'reference':
        return { label: t('reference'), count: referenceTodos.length, icon: FileText, color: isDarkMode ? 'text-blue-400' : 'text-blue-500', bgColor: isDarkMode ? 'bg-blue-900' : 'bg-blue-50', borderColor: 'border-blue-500' };
    }
  };
    ...
    <div className="flex-1 flex flex-col h-full safe-area-bottom">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              title={`${t('sortBy')}${sortField === 'createdAt' ? t('createdAt') : t('completedAt')}${sortOrder === 'asc' ? t('sortAsc') : t('sortDesc')}`}
            >
              <ArrowUpDown className="w-4 h-4" />
              <span className="hidden sm:inline">
                {sortField === 'createdAt' ? t('createdAt') : t('completedAt')}
                {sortOrder === 'asc' ? '↑' : '↓'}
              </span>
            </button>
            <button
      ...
      <div className={`flex border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
        ...
        >
          ...
          <span>{t('pending')} ({pendingTodos.length})</span>
        </button>
        <button
        ...
        >
          ...
          <span>{t('reference')} ({referenceTodos.length})</span>
        </button>
      </div>
        ...
        {currentTodos.length === 0 ? (
          <div className={`flex flex-col items-center justify-center h-full ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>
            ...
            <p className="text-xs sm:text-sm">
              {activeTab === 'pending' ? t('noPendingTodos') : 
               activeTab === 'completed' ? t('noCompletedTodos') : t('noReferenceTodos')}
            </p>
          </div>
        ) : (
